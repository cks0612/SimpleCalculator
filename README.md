...
-구현내용과기능설명
 ...
 -숫자 버튼(1,2,3,4,5,6,7,8,9,0) 클릭시 텍스트 박스 1,2 에 숫자가 입력된다.
 
 -텍스트 박스 1 에는 숫자와 연산자 포함 모든 전체 식이 입력된다.

 -텍스트 박스 2 에는 마지막 덩어리의 숫자만 입력된다. ex) 100 + 100 ->100 만 보임 
 
 -사칙연산중 + 버튼의 기능이 구현됨 1+2 = 3 으로 계산
 
 -= 버튼 클릭시 전체 식이 계산되어 결과가 = 뒤에 출력됨
 
 -= 입력시 텍스트 박스 2 에는 결과 값만 보임 100 + 100 = 입력시 200 만 보임

 -= 입력시 텍스트 박스 1 에는 전체 식이 보임 ex) 100 + 100 = 200 전부 보임 
